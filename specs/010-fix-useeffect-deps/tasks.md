...
- [ ] T009 [US1] 新規追加したテスト（T005）が成功することを確認
- [ ] T010 [US1] 既存のすべてのテストが成功することを確認（リグレッションチェック）
- [ ] T011 [US1] Greenフェーズのコミット: `feat(US1): useEffect依存配列からsubState.subscriptionsを削除`

### Refactor for User Story 1 (Refactor フェーズ) ♻️

> **テストを通したまま品質向上**: 重複排除、意図の明確化

- [ ] T012 [US1] コメントを更新し、意図を明確化（必要に応じて）
- [ ] T013 [US1] テストコードのリファクタリング（必要に応じて）
- [ ] T014 [US1] ESLint警告がないことを確認（npm run lint）
- [ ] T015 [US1] TypeScript型チェックが成功することを確認（npm run type-check）
- [ ] T016 [US1] Refactorフェーズのコミット: `refactor(US1): コメントを更新し意図を明確化`
...
- [ ] T017 [US2] 既存の手動更新テストを確認し、フィード取得が1回のみ実行されることを検証するアサーションを追加 in frontend/tests/integration/refreshFlow.test.tsx
- [ ] T018 [US2] テストを実行し、すべて成功することを確認（既存機能が壊れていないことの検証）
- [ ] T019 [US2] テストコミット: `test(US2): 手動更新機能のフィード取得回数検証を追加`

### Verification for User Story 2 ✅

> **検証のみ**: 実装変更は不要（User Story 1の修正が手動更新に悪影響を与えていないことを確認）

- [ ] T020 [US2] 手動更新機能（handleRefresh）が正しく動作することを手動確認
- [ ] T021 [US2] 更新中のUI状態（ローディングインジケーター、ボタン無効化）が正しく動作することを手動確認
- [ ] T022 [US2] すべてのテストが成功することを確認
...
- [ ] T023 [P] フィードが0件の状態でページをリロードした場合、不要なAPIリクエストが発生しないことを検証するテストを frontend/tests/integration/feedFlow.test.tsx に追加
- [ ] T024 [P] フィード削除時に削除されたフィードへのリクエストが発生しないことを検証するテストを frontend/tests/integration/feedFlow.test.tsx に追加
- [ ] T025 [P] カスタムタイトル編集時に不要な再フェッチが発生しないことを検証するテストを frontend/tests/integration/feedTitleFlow.test.tsx で確認または追加

### Verification for Edge Cases ✅

- [ ] T026 すべてのエッジケーステストが成功することを確認
- [ ] T027 既存のすべての統合テストが成功することを確認（完全なリグレッションチェック）
- [ ] T028 エッジケーステストのコミット: `test(edge): エッジケースの検証テストを追加`
...
- [ ] T029 [P] テストが高速に実行されることを確認（全テストが数秒以内に完了）
- [ ] T030 [P] テストが独立していることを確認（実行順序に依存しない）
- [ ] T031 [P] テストが反復可能であることを確認（何度実行しても同じ結果）
- [ ] T032 [P] テストが自己検証可能であることを確認（成功/失敗が自動判定）

### Coverage & Performance

- [ ] T033 [P] テストカバレッジを確認し、修正部分が100%カバーされていることを検証（憲法: 変更部分は100%カバー）
- [ ] T034 [P] パフォーマンステストを実行し、SC-001（1回のみのAPIリクエスト）が達成されていることを手動確認

### Documentation & Quality Gates

- [ ] T035 コードレビュー用にコミット履歴を確認（Red → Green → Refactor のサイクルが明確か）
- [ ] T036 SPECIFICATION.mdを更新し、この修正を反映（CLAUDE.mdの指示に従う）
- [ ] T037 すべてのQuality Gates（憲法のマージ前の必須条件）をパス確認：
  - [ ] すべてのテストがパス
  - [ ] カバレッジが基準（80%）以上
  - [ ] TypeScript の型チェックがパス
  - [ ] ESLint の警告ゼロ
- [ ] T038 最終コミット: `docs: SPECIFICATION.mdを更新`
