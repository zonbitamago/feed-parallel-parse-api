...
- ✅ **自動更新機能（ポーリング）**: 10分間隔で新着記事を自動検出・通知
  - バックグラウンドでフィードを自動取得
  - 新着記事があれば画面上部に通知バナー表示
  - ワンクリックで新着記事を読み込み
  - ポーリング状態をリアルタイム表示（最終取得時刻、次回取得まで）
  - オンライン時のみ動作、オフライン復帰時に自動再開
- ✅ **In-Appチュートリアル**: 初回訪問時にアプリの使い方をガイド
  - 7ステップのインタラクティブなウォークスルー
  - ヘルプボタンからいつでも再表示可能
  - モバイル・タブレット・デスクトップ完全対応
  - WCAG 2.1 AA準拠（キーボード操作、スクリーンリーダー対応）
- ✅ **リアルタイム検索**: 記事のタイトルや要約から素早く検索（300msデバウンス）
- ✅ **フィード購読管理**: URL追加・削除、最大100件まで購読可能
- ✅ **手動更新**: ワンクリックで全フィードを即座に更新
...
- date-fns 4.1（日付処理、相対時刻表示）
- driver.js 2.1（In-Appチュートリアル、5 kB gzip）
- vite-plugin-pwa 1.1（PWA対応）
- workbox-window（Service Worker管理）
- **テストカバレッジ**: 49ファイル・347テスト合格（343合格、4スキップ）
